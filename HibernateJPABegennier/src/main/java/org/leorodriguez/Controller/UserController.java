...
public class UserController {
    public static void main(String[] args) {
       ...
       // addUser(factory,session);
        ...
        //getmaxSalaryGroupBy();
        //namedQueryExample(session, 1);
        getRecordById(factory, session,2);
    }
    public static void getRecordById(SessionFactory factory, Session session, int minId) {
        String hql = "FROM User u WHERE u.id > :minId ORDER BY u.salary DESC";
        TypedQuery<User> query = session.createQuery(hql, User.class);
        query.setParameter("minId", minId);  // Using dynamic parameter
    ...
    public static void namedQueryExample(Session session, int userId) {
        String hql = "FROM User u WHERE u.id = :id";
        TypedQuery<User> query = session.createQuery(hql, User.class);
        query.setParameter("id", userId);  // Using dynamic parameter
    ...
    public static void getRecordById(SessionFactory factory, Session session) {
        ...
        List<User> results = query.getResultList();
        System.out.printf(
                "%s%13s%17s%34s%21s%n", "|User Id", "|Full name", "|Email", "|Password", "|Salary");
        for (User u : results) {
            System.out.printf(
                    " %-10d %-20s %-30s %-23s %s %n", u.getId(), u.getFullName(), u.getEmail(), u.getPassword(), u.getSalary());
        }
    }


    public static void findUserHql(SessionFactory factory, Session session) {
        String hqlFrom = "FROM User"; //Gets all from user class
